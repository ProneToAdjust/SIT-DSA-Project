    ...
    def on_button_click(self):
        ...
        try:
            ...
            if len(startcoord) == 2 and len(endcoord) == 2:
                ...
                for count, route in enumerate(data):
                    locations = []
                    walking = []
                    ...
                    if route["bus_service"] != "walk":
                        ...
                        for i in range(0, len(route["nodes"])):
                            ...
                            if not (  # only if previous step was not walk and the previous step's node destination and current step's node origin are not the same
                                data[count - 1]["bus_service"] == "walk"
                                and data[count - 1]["nodes"][1] == route["nodes"][i]
                            ):
                                folium.Marker(
                                    [
                    ...
                    # for walking points
                        ...
                        for i in range(0, len(route["nodes"])):
                            ...
                            # place a marker at each walking locations if
                            folium.Marker(
                                [
                                    route["nodes"][i].coordinates[0],
                                    route["nodes"][i].coordinates[1],
                        ...
                        for i, node in enumerate(route["nodes"]):
                            ...
                            route_coordinates = routing_walking["features"][0][
                                "geometry"
                            ]["coordinates"]
